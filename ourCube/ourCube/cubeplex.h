...
#define BUFFERSIZE 192 // 192 b/c 64 LEDs, each with 3 legs --> 64x3=192
#include "Arduino.h"
#include "mappings.h"
#include "niceTimer.h"
...
/* Buffer has three sections of 64 slots: one for each primary color
  to get secondary color, change more than one slot

  HYPOTHESIS: 
  changing x moves your x coordinate -- which of the four columns the target spire is in
  changing y chooses the correct spire from the columnn of 4 specified by x
  changing z moves you up/down a single spire to the correct LED */
...
void flushElement(_frame_light* &copy_frame,int pin1,int pin2,int brightness) {
  pin1--;
  pin2--;
  // copy_frame    copy_frame_nextdoor->next = copy_frame, pin1=pin1.., pin2=pin2...
